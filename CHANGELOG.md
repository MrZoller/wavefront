...
- **Reduced-motion support (`prefers-reduced-motion`).** Auto-animating modules now respect the
  ...
  control, so motion is opt-in; with it **off**, behavior is unchanged (the phasor spins, the
  front-door "this is alive" cue stays). The frames are chosen to teach the concept at rest — the
  **rotating phasor** rests mid-rotation (arrow at 45°, I/Q waveforms drawn, I/Q/θ readouts
  ...
  sliders still live, and gains a reduced-motion-only control to play the spin on demand (normal mode
  stays byte-identical); the **adaptive equalizer** rests on its converged frame (taps learned, error
  fallen) with Reset to replay the run. Enabling Reduce Motion **mid-session** pauses a running module
  without a reload, and never force-resumes when it's switched back off, so a manual Pause/Resume
  choice is preserved. Two shared hooks in `src/components/plots/usePrefersReducedMotion.ts` —
  `usePrefersReducedMotion` (tracks the setting, updating live) and `useReducedMotionPlayState` (the
  reduced-motion-aware play-state) — carry the convention so future animated modules inherit it by
  calling them. A global `@media (prefers-reduced-motion: reduce)` rule in `index.css` collapses the
  decorative CSS layer (hover/colour transitions, the slider-thumb halo, any future keyframe
  animation). No DSP, computed values, or copy changed — only the default play-state, the resting
  frame, and the opt-in affordance under the preference.
