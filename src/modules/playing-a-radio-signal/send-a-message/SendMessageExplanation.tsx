    ...
    <GlossedText>
      <div className="flex flex-col gap-4">
        <section>
          ...
          <p>
            Everything from this track, wired together: each character becomes 8 bits, the bits
            group into constellation symbols, the symbols cross an AWGN channel, and the receiver
            slices each one to its nearest point and reassembles the text. This is a (simplified)
            modem.
          </p>
        </section>
        ...
        <section>
          ...
          <p>
            Below a certain Eb/N0 the bit error rate climbs fast (the &ldquo;waterfall&rdquo;), so
            the message goes from perfect to unreadable over just a few dB. A single flipped bit can
        ...
        <section className="rounded-md border border-border bg-surface-raised p-3 text-xs text-text-faint">
          Uncoded and hard-decision, with perfect synchronization — coding, equalization, and sync
          are later tracks. All signals are synthetic.
        </section>
      </div>
    </GlossedText>
  );
