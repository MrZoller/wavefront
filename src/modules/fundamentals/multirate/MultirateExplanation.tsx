  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <section>
        ...
        <p>
          Dropping samples shrinks the <Term id="nyquist">Nyquist</Term> limit, so anything above
          the new limit <Term id="aliasing">aliases</Term> into the band — irreversibly. The fix is
          to low-pass to the new Nyquist <em>before</em> decimating. Interpolation has the mirror
          problem: zero-stuffing creates spectral images that a low-pass must remove.
        </p>
      </section>

      <section className="rounded-md border border-border bg-surface-raised p-3 text-xs text-text-faint">
        This decimate = low-pass + downsample is exactly the back half of a{' '}
        <Term id="ddc">digital downconverter</Term> — the channelizer's building block. All signals
        are synthetic.
      </section>
    </div>
  );
