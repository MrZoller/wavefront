    ...
    <GlossedText>
      <div className="flex flex-col gap-4">
        ...
        <section>
          ...
          <p>
            In 2D, three receivers give only <em>two</em> independent hyperbolas — and two curves
            can cross at <em>two</em> points, so a single extra intersection survives. A fourth
            receiver adds a third hyperbola that passes through only the true point, removing the
            ambiguity (this scene ships with four). We then recover the position by Gauss–Newton
            least squares on the range-difference equations — watch the fix track the emitter as you
            drag.
          </p>
        </section>

        <section className="rounded-md border border-border bg-surface-raised p-3 text-xs text-text-faint">
          How well the curves cross depends on receiver geometry — the next module makes that
          precise. All scenarios here are synthetic.
        </section>
      </div>
    </GlossedText>
  );
