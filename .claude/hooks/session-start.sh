...
# Note: background execution is enabled by "async": true in .claude/settings.json — the
# hook's stdout is only parsed after it exits, so it cannot opt into async itself.
